...
public class MaestroSettings {
    ...
    public MaestroSettings()
    {
        ...
        servosAvailable = 6;
        servoPeriod = 156;
        serialMode = SerialMode.UART_DETECT_BAUD_RATE;
        baudRate = 9600;
        enableCrc = false;
        neverSuspend = false;
        deviceNumber = 12;
    ...
    public ChannelSettings getChannel(int port) {
        if (!channels.containsKey(port))
            return ChannelSettings.CreateDefault();

        return channels.get(port);
    }
    public void AddChannel(int port, ChannelSettings settings){
        if(channels.containsKey(port))
    ...
    public boolean isSimilarTo(MaestroSettings settings){
        ...
        for (Map.Entry<Integer, ChannelSettings> channel : channels.entrySet())
        {
